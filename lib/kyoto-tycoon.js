...
KyotoTycoon.prototype.play_script = function (name) {
    var opt = {};
    opt.name = name;
    var len = arguments.length
    var hash = (arguments[1] && typeof arguments[1] == 'object') ? arguments[1] : null; 
    var callback = (typeof arguments[len - 1] == 'function') ? arguments[len - 1] : null;
    if (hash) {
        for (var k in hash) {
            if (hash.hasOwnProperty(k)) {
    ...
    this.tsvrpc.rpc('play_script', opt, function (err, data) {
        var r = {}
        for (var k in data) {
            if (k.match(/^_/)) {
                r[k.replace(/^_/, '')] = data[k]
